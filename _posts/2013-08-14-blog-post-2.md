...
- *There is a theorem which states the following dichotomy holds : 
  1. if \\(\mathbb{P}_{i}(T_{i}<\infty)=1\\), then \\(i\\) is recurrent and \\(\sum\limits_{n\in\mathbb{N}}p^{(n)}_{ii}\to\infty\\),
  2. if \\(\mathbb{P}_{i}(T_{i}<\infty)<1\\), then \\(i\\) is transient and \\(\sum\limits_{n\in\mathbb{N}}p^{(n)}_{ii}<\infty\\).
